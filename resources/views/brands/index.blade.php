    ...
    <div x-data="modalHandler" class="relative">
        <div class="container mx-auto py-4">
            <div class="flex justify-between items-center mb-4">
                <div>
                    <h1 class="text-xl font-bold">Item Brands</h1>
                    <p class="text-gray-600 text-sm">All your item brands in one place. Quickly add, edit, or organize them as needed.</p>
                </div>
                ...
                <table class="min-w-full table-auto text-sm">
                    <thead>
                        <tr>
                            ...
                            <th class="px-4 py-4 text-left w-[30%]">Brand Name</th>
                    ...
                    <tbody>
                        @forelse ($brands as $index => $brand)
                            <tr class="border-t border-gray-200 hover:bg-gray-50">
                                <td class="px-4 text-center">{{ $brands->firstItem() + $index }}</td>
                                <td class="px-4 ">{{ $brand->name }}</td>
                                <td class="px-4 py-1 flex space-x-2 justify-center">
                                    ...
                                    <button 
                                        ...
                                        @click="window.dispatchEvent ( new CustomEvent ('open-modal', {
                                            detail: {
                                                title: 'Delete Brand',
                                                content: 'Are you sure you want to delete <b>{{ $brand->name }}</b> from list?',
                                                cancelText: 'Cancel',
                                                confirmText: 'Delete',
                                                confirmColor: 'red',
                                                actionURL: '{{ route('brands.destroy', $brand->id) }}',
                                                actionMethod: 'DELETE'
                                            }
                                        }))"
                                    >
